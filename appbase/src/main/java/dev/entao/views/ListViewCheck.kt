...
fun ViewGroup.listViewCheck(block: ListViewCheck.() -> Unit): ListViewCheck {
    return append {
        this.block()
    }
}

class ListViewCheck(context: Context) : ListViewX(context) {
    ...
    override fun onClickAdapter(position: Int, view: View, item: Any) {
        if (checkCallback.onItemCheckable(item)) {
            ...
            super.onClickAdapter(position, view, item)
        }
    }
